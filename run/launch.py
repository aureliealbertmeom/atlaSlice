#! /usr/bin/env python
# This script makes sure that the required operation is possible
# and launch the appropriate script
...
from params import plots_dict as pparams
from params import slice_dict as sparams

def parse_args():
    parser=argparse.ArgumentParser(description="check operation definition and generate the associated job")
    parser.add_argument('-param',type=str,help='operation param')
...
def check(machine,configuration,simulations,regions,variables,frequency,date_init,date_end,operation):
    ...
    for sim in simulations:
        for var in variables:
            f.check(frequency,params.frequencies[configuration][sim][var],'The variable '+str(var)+' for the simulation '+str(sim)+' does not have the frequency '+str(frequency))

    for sim in simulations:
        if pd.Timestamp(date_init) < pd.Timestamp(params.sim_date_init[configuration][sim]):
            sys.exit('The initial date '+str(date_init)+' is not included the period of output of the simulation '+str(sim))
        if pd.Timestamp(date_end) > pd.Timestamp(params.sim_date_end[configuration][sim]):
            sys.exit('The end date '+str(date_end)+' is not included the period of output of the simulation '+str(sim))

    print('All checks have passed, we are now going to generate and launch a job that will perform the operation '+str(operation)+' on variables '+str(variables)+' from simulations '+str(simulations)+' from configuration '+str(configuration)+' from '+str(date_init)+' to '+str(date_end)+' at '+str(frequency)+' frequency on machine '+str(machine))
...
def main():
    ...
    #Check if the defined operations are possible
    ...
    nb_operation = len(da.simulations) * len(da.regions) * len(da.variables) * len(da.extra_params) * f.nb_time_steps(da.date_init,da.date_end,da.frequency
    match da.operation:
        case 'map'|'map_noproj'|'section'|'time_series'|'hovmuller'|'map_orthoa'|'map_orthoi'|'map_moll':
            launch_atlas.py 
        case 'extract'|'degrad3'|'degrad10'|'daily_mean'|'daily_files'|'apply_mask'|'project_sosie':
            launch_slice.py
