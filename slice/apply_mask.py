...
def mask(machine,configuration,simulations,regions,variables,frequency,date_init,date_end):
    #Concatenate the name of all simulations, regions, variables
    allsimulations=f.concatenate_all_names_in_list(simulations)
    allregions=f.concatenate_all_names_in_list(regions)
    allvariables=f.concatenate_all_names_in_list(variables)
    ...
    mpmdname='tmp_mpmd_apply_mask_'+str(machine)+'_'+str(configuration)+'_'+str(allsimulations)+'_'+str(allregions)+'_'+str(allvariables)+'_'+str(frequency)+'_'+str(date_init)+'_'+str(date_end)+'.ksh'
    jobname='tmp_job_apply_mask_'+str(machine)+'_'+str(configuration)+'_'+str(allsimulations)+'_'+str(allregions)+'_'+str(allvariables)+'_'+str(frequency)+'_'+str(date_init)+'_'+str(date_end)+'.ksh'
    
    shutil.copyfile('job_'+str(machine)+'_template.ksh',jobname)
    subprocess.call(["sed", "-i", "-e",  's/MPMDCONF/'+str(mpmdname)+'/g', jobname])

    #Loop over all the simulations, regions, variables and dates of the requested operations
    nb_procs=0
    nb_proc_max=params.mprocs['mask']
    nb_jobs=1

    all_dates=pd.date_range(date_init,date_end,freq='D')
    for var in variables:
        if params.vars_dim[var]=='2D':
            print('We are going to mask variable '+str(var)+' month by month')
            all_month=pd.date_range(date_init,date_end,freq='M')
            for ym in all_month:
                year=ym.year
                month=ym.month
                ...
                for simulation in simulations:
                    for region in regions:
                        scriptname=('tmp_script_mask_'+str(machine)+'_'+str(configuration)+'_'+str(simulation)+'_'+str(region)+'_'+str(var)+'_'+str(frequency)+'_'+str(tag)+'.ksh')
                        f.use_template('script_mask_2Dvar_1month_template.ksh', scriptname, {'CONFIGURATION':str(configuration),'SIMULATION':str(simulation),'REGIONABR':str(params.ex[configuration][region]), 'REGIONNAME':str(region),'VARIABLE':str(var), 'VNAME':str(params.vars_name[simulation][var]),'FREQUENCY':str(frequency), 'YEAR':str(year), 'MONTH':str(mm), 'STYLENOM':str(params.stylenom[machine][configuration][simulation]),'SCPATH':str(params.scratch_path[machine]),'SOSIEPATH':str(params.sosie_path[machine]),'MASKNAME':params.mask2Dname[var],'MASKFILE':params.mask[reg]})
                        subprocess.call(["chmod", "+x", scriptname])
                        ...
                        if nb_procs == nb_proc_max:
                            ...
                            nb_jobs=nb_jobs+1
                            nb_procs=0
                            mpmdname='tmp_mpmd'+str(nb_jobs)+'_apply_mask_'+str(machine)+'_'+str(configuration)+'_'+str(allsimulations)+'_'+str(allregions)+'_'+str(allvariables)+'_'+str(frequency)+'_'+str(date_init)+'_'+str(date_end)+'.ksh'
                            jobname='tmp_job'+str(nb_jobs)+'_apply_mask_'+str(machine)+'_'+str(configuration)+'_'+str(allsimulations)+'_'+str(allregions)+'_'+str(allvariables)+'_'+str(frequency)+'_'+str(date_init)+'_'+str(date_end)+'.ksh'
...
def main():
    ...
    mask(da.machine,da.configuration,da.simulations,da.regions,da.variables,da.frequency,da.date_init,da.date_end)
