...
def check(machine,config,simulations,variables,plot_type,plot_locs,plot_regions,frequency,date_init,date_end):
    ...
    match plot_type:
        case 'map_noproj' | 'map_moll' | 'map_orthoa' | 'map_orthoi' | 'hovmuller' | 'zyplot':
...
def job(machine,config,simulations,variables,plot_type,plot_locs,plot_regions,frequency,date_init,date_end):
    ...
    if plot_type[:3] == 'map' or plot_type == 'time_series' or 'hovmuller':
    ...
    #When we plot zy plots no locs are defined, only the region
    elif plot_type == 'zyplot':
        allregions=f.concatenate_all_names_in_list(plot_regions)
        mpmdname='tmp_mpmd_'+str(machine)+'_'+str(config)+'_'+str(allsimulations)+'_'+str(allvariables)+'_'+str(allregions)+'_'+str(plot_type)+'_'+str(frequency)+'_'+str(f.tag_from_string_date(date_init,params.stylenom[machine][config][simulations[0]]))+'-'+str(f.tag_from_string_date(date_end,params.stylenom[machine][config][simulations[0]]))+'.conf'
        jobname='tmp_job_plot_'+str(machine)+'_'+str(config)+'_'+str(allsimulations)+'_'+str(allvariables)+'_'+str(allregions)+'_'+str(plot_type)+'_'+str(frequency)+'_'+str(f.tag_from_string_date(date_init,params.stylenom[machine][config][simulations[0]]))+'-'+str(f.tag_from_string_date(date_end,params.stylenom[machine][config][simulations[0]]))+'.ksh'
    ...
    #No loop over time for time-series and hovmuller, we need to access the whole period
                    ...
                    for loc in plot_locs:
                        for reg in plot_regions:
                            if plot_type[:3] == 'map':
                                ...
                                f.use_template('script_plot_'+str(machine)+'.ksh',scriptname, {'MACHINE':str(machine),'CONFIGURATION':str(config),'SIMULATION':str(sim),'VARIABLE':str(var),'PLOTREGION':str(reg),'TYPE':str(plot_type),'LOCATION':str(loc),'FREQUENCY':str(frequency),'DATE':str(date),'SCRIPTDIR':str(params.script_path[machine])})
                            elif plot_type == 'section':
                                ...
                                f.use_template('script_plot_'+str(machine)+'_noreg.ksh',scriptname, {'MACHINE':str(machine),'CONFIGURATION':str(config),'SIMULATION':str(sim),'VARIABLE':str(var),'TYPE':str(plot_type),'LOCATION':str(loc),'FREQUENCY':str(frequency),'DATE':str(date),'SCRIPTDIR':str(params.script_path[machine])})
                            elif plot_type == 'zyplot':
                                scriptname='tmp_script_plot_'+str(machine)+'_'+str(config)+'_'+str(sim)+'_'+str(var)+'_'+str(reg)+'_'+str(plot_type)+'_'+str(frequency)+'_'+str(tag)+'.ksh'
                                f.use_template('script_plot_'+str(machine)+'_noloc.ksh',scriptname, {'MACHINE':str(machine),'CONFIGURATION':str(config),'SIMULATION':str(sim),'VARIABLE':str(var),'PLOTREGION':str(reg),'TYPE':str(plot_type),'FREQUENCY':str(frequency),'DATE':str(date),'SCRIPTDIR':str(params.script_path[machine])})
